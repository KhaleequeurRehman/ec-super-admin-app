...
import { useGetNotificationHistoryOfCustomerMutation, useGetNotificationHistoryOfCustomerQuery } from "api/notification";
...
const SendNotificationCustomerDataGrid = ({ IdSubColor, setOpenCustomerDetail }) => {

  const [page, setPage] = useState(1);
  const [size, setSize] = useState(10);
  const [totalNotificatonsCount, setTotalNotificatonsCount] = useState(1);

  const [getNotificationHistoryOfCustomer, resdata] = useGetNotificationHistoryOfCustomerMutation()

  // const {data:notificationHistoryOfCustomerData,isLoading,refetch,isSuccess} = useGetNotificationHistoryOfCustomerQuery(`page=${page}&size=${size}&sortBy=createdAt`)

  const [notificationHistoryOfCustomer, setNotificationHistoryOfCustomer] = useState([])
  ...
  //   setNotificationHistoryOfCustomer(notificationHistoryOfCustomerData?.allNotificationHistoryList)
  //   setTotalRecordsCount(notificationHistoryOfCustomerData?.totalNotificatonsCount)
    
  // }, [notificationHistoryOfCustomerData])


  const getNotificationHistoryOfCustomerFunc = () => {
    getNotificationHistoryOfCustomer({page:page,size:size}).then((res) => {
      if(res.data){
        console.log("res.data getNotificationHistoryOfCustomerFunc ", res?.data);
        // setNotificationHistoryData(res?.data)
        setNotificationHistoryOfCustomer(res?.data)
  ...
  useEffect(() => {
    getNotificationHistoryOfCustomerFunc()
  }, [page])
            ...
            <>
              <Box sx={{ width: "100%" }}>
                <StyledDataGrid
                  ...
                  // rows={notificationHistoryOfCustomer || []}
                  // rows={notificationHistoryOfCustomerData?.allNotificationHistoryList || []}
                  rows={notificationHistoryOfCustomer?.allNotificationHistoryList || []}
...
export default SendNotificationCustomerDataGrid;
