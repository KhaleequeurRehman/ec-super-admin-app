...
import { useGetNotificationHistoryOfCatererMutation, useGetNotificationHistoryOfCatererQuery} from "api/notification";
...
const SendNotificationCatererDataGrid = ({ IdSubColor, setOpenCustomerDetail }) => {

  const [page, setPage] = useState(1);
  const [size, setSize] = useState(10);
  const [totalNotificatonsCount, setTotalNotificatonsCount] = useState(1);

  // const {data:notificationHistoryOfCatererData,isLoading,refetch,isSuccess} = useGetNotificationHistoryOfCatererQuery(`page=${page}&size=${size}&sortBy=createdAt`)
  const [getNotificationHistoryOfCaterer, resdata] = useGetNotificationHistoryOfCatererMutation()

  const [notificationHistoryOfCaterer, setNotificationHistoryOfCaterer] = useState([])
  ...
  //   setNotificationHistoryOfCaterer(notificationHistoryOfCatererData?.allNotificationHistoryList)
  //   setTotalRecordsCount(notificationHistoryOfCatererData?.totalNotificatonsCount)
    
  // }, [notificationHistoryOfCatererData])

  const getNotificationHistoryOfCatererFunc = () => {
    getNotificationHistoryOfCaterer({page:page,size:size}).then((res) => {
      if(res.data){
        console.log("res.data getNotificationHistoryOfCatererFunc ", res?.data);
        // setNotificationHistoryData(res?.data)
        setNotificationHistoryOfCaterer(res?.data)
  ...
  useEffect(() => {
    getNotificationHistoryOfCatererFunc()
  }, [page])
            ...
            <>
              <Box sx={{ width: "100%" }}>
                <StyledDataGrid
                  ...
                  // rows={notificationHistoryOfCaterer || []}
                  // rows={notificationHistoryOfCatererData?.allNotificationHistoryList || []}
                  rows={notificationHistoryOfCaterer?.allNotificationHistoryList || []}
...
export default SendNotificationCatererDataGrid;
