...
const UpdateCuisine = ({handleAddModalClose,refetchAllCuisinesData,selectedCusine}) => {

  const [updateCuisine, { isLoading }] = useUpdateCusineMutation()
  const [image, setImage] = useState("assets/images/image profile.svg");
    ...
    onSubmit: (values, action) => {
      console.log("UpdateCuisine Form => ", values);
      console.log("selectedCusine => ", selectedCusine);

      let formData = new FormData(); //formdata object
      formData.append("id", selectedCusine?._id);
      Object.keys(values).map((keys) => {
       formData.append(keys, values[keys]);
    });
    
        // formData.append("id", seletedCuisine?._id);

      updateCuisine(formData).then((res) => {
        if (res.data) {
  ...
  return (
    <>
      <Grid container justifyContent="center" alignItems="center">
        <Grid item xs={12}>
            <Typography sx={{color:"#9EA3AE",fontSize:"16px",fontWeight:"400",mt:"6px"}}>Update Cuisine</Typography>
            <Box component="form" onSubmit={formik.handleSubmit}  autoComplete="off">
              ...
              <Box sx={{mt:"40px",mb:"20px"}}>
                ...
                }}} variant="contained" disabled={isLoading} fullWidth >Update Cuisine</Button>
              </Box>
            </Box>
        </Grid>
      </Grid>
