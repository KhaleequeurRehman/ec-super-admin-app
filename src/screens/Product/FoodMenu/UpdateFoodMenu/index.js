...
import { useAddMealplanMutation, useUpdateMealplanMutation } from "../../../../api/mealplan";
...
const UpdateFoodMenu = ({handleUpdateModalClose,selectedMealplan,refetchAllFoodMenusData}) => {

  const [getAllCusines,resdata] = useGetAllCusinesMutation()
  const [cusines, setCusines] = useState("");

  const [updateMealplan, { isLoading }] = useUpdateMealplanMutation()
  const [image, setImage] = useState("assets/images/image profile.svg");

    const formik = useFormik({
    initialValues: {
        name: selectedMealplan?.data?.name || "",
        description: selectedMealplan?.data?.description || "",
        // endDate: new Date() || dayjs('2014-08-18T21:11:54'),
        endDate: dayjs( selectedMealplan?.data?.endDate || '2023-02-18T21:11:54'),
        reviewStatus: selectedMealplan?.data?.reviewStatus || false,
        mealCourse: "",
        addOnes: "",
        forReview: selectedMealplan?.data?.forReview || false,
        cuisine: "",
        image: "",
    },
    ...
    onSubmit: (values, action) => {
      console.log("Update Menu Form => ", values);

      let formData = new FormData(); //formdata object
      formData.append("id",selectedMealplan?._id);
      Object.keys(values).map((keys) => {
       formData.append(keys, values[keys]);
      });

      updateMealplan(formData).then((res) => {
        if (res.data) {
  ...
  console.log("selectedMealplan update mealplan ",selectedMealplan)
  console.log("cusines update mealplan ",cusines)

  return (
    <>
      <Grid container justifyContent="center" alignItems="center">
        <Grid item xs={12}>
            <Typography sx={{color:"#9EA3AE",fontSize:"16px",fontWeight:"400",mt:"6px"}}>Update Menu</Typography>
            <Box component="form" onSubmit={formik.handleSubmit}  autoComplete="off">
              ...
              <Box sx={{mt:"40px",mb:"20px"}}>
                ...
                }}} variant="contained" disabled={isLoading} fullWidth >Update Menu</Button>
              </Box>
            </Box>
        </Grid>
      </Grid>
...
// import { useUpdateMealplanMutation } from "../../../../api/mealplan";
