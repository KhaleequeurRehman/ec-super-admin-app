...
import { useUpdateSubCusineMutation } from "../../../../api/subCuisine";
...
const UpdateSubCuisine = ({handleAddModalClose,refetchAllSubCuisinesData,seletedSubCuisine}) => {

  const [updateSubCuisine, { isLoading }] = useUpdateSubCusineMutation()
  const [image, setImage] = useState("assets/images/image profile.svg");
    ...
    onSubmit: (values, action) => {
      console.log("UpdateSubCuisine Form => ", values);
      ...
      Object.keys(values).map((keys) => {
       formData.append(keys, values[keys]);
        ...
        formData.append("id", seletedSubCuisine?._id);

      updateSubCuisine(formData).then((res) => {
        if (res.data) {
          console.log("updated successfully");
          handleAddModalClose();
          refetchAllSubCuisinesData();
        }
      });
      action.resetForm();
    },
  ...
  return (
    <>
      <Grid container justifyContent="center" alignItems="center">
        <Grid item xs={12}>
            <Typography sx={{color:"#9EA3AE",fontSize:"16px",fontWeight:"400",mt:"6px"}}>Update Sub Cuisine</Typography>
            <Box component="form" onSubmit={formik.handleSubmit}  autoComplete="off">
              ...
              <Box sx={{mt:"40px",mb:"20px"}}>
                ...
                }}} variant="contained" disabled={isLoading} fullWidth >Update Sub Cuisine</Button>
              </Box>
            </Box>
        </Grid>
      </Grid>
...
export default UpdateSubCuisine;