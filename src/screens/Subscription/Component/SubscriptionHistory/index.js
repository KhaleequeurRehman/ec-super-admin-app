...
const SubscriptionHistory = ({setIsShownSubscriptionHistory}) => {
  
  const navigate = useNavigate();

  const [page, setPage] = useState(1);
  const [size, setSize] = useState(10);
  const [totalSubscriptionHistoryCount, setTotalSubscriptionHistoryCount] = useState(1);

  // const [getSubscriptions, resdata] = useGetSubscriptionsMutation()
  const [getSubscriptionHistory, resdata] = useGetSubscriptionHistoryMutation()

  // const [searchSubscriptions, respdata] = useSearchSubscriptionsMutation()
  const [searchSubscriptions, respdata] = useSearchSubscriptionHistoryMutation()
...
const getSubscriptionHistoryData = ()=> {
  getSubscriptionHistory({ page:page, size:size,status:"all" }).then((res) => {
    if (res.data){
      console.log("res.data getSubscriptionHistoryData ", res?.data);
      setSubscriptionData(res?.data)
      setTotalSubscriptionHistoryCount(res?.data?.totalSubscriptionHistoryCount)
    }
  });
}

const getLastAddedSubscriptionHistoryData = ()=> {
  getSubscriptionHistory({ page:page, size:size,status:"all",sortBy:"createdAt",orderBy:"desc" }).then((res) => {
    if (res.data){
      console.log("res.data getLastAddedSubscriptionHistoryData ", res?.data);
      setSubscriptionData(res?.data)
      setTotalSubscriptionHistoryCount(res?.data?.totalSubscriptionHistoryCount)
    }
  });
...
const filterSubscriptionHistoryWithStatusData = (status)=> {
  getSubscriptionHistory({ page:page, size:size,status:status }).then((res) => {
    if (res.data){
      console.log("res.data filterSubscriptionHistoryWithStatusData ", res?.data);
      setSubscriptionData(res?.data)
      setTotalSubscriptionHistoryCount(res?.data?.totalSubscriptionHistoryCount)
    }
  });
...
const handleClose = (data) => {
  if(data === "cancel" || data === "pause" || data === "active" || data === "request"){
    filterSubscriptionHistoryWithStatusData(data)
  }else if(data === "all"){
...
const searchSubscriptionHistoryFunc = (newValSearch)=> {
  if(newValSearch === ""){
    getSubscriptionHistoryData()
    ...
    searchSubscriptions({ search: newValSearch }).then((res) => {
      if (res.data){
        console.log("searchSubscriptionHistory res.data SubscriptionHistoryData ", res?.data);
        setSubscriptionData(res?.data)
        setTotalSubscriptionHistoryCount(res?.data?.totalSubscriptionHistoryCount)
...
//   );
  ...
  return (
    <>
      <Box width="100%">
        <CustomBreadcrumbs
        typographyText="Subscription History"
        breadcrumbsLinksArr={breadcrumbsLinksDataArr}
        clickHandler={()=> setIsShownSubscriptionHistory(false)}
        />
        <Box sx={{my:"10px"}}>
            {
                resdata?.isLoading ? (
                ...
                <Box>
                    ...
                    >
                        ...
                        <SearchInputField
                            ...
                            onChange={
                              (e) => {
                                ...
                                searchSubscriptionHistoryFunc(e.target.value)
                              }
                            }
                            InputProps={{
                            endAdornment: (
                        ...
                        <Box sx={{display:'flex'}}>
                          <Button variant="text" onClick={getLastAddedSubscriptionHistoryData}>
                ...
                >
                    ...
                    <>
                        ...
                        <StyledDataGrid
                            ...
                            rows={subscriptionData?.subscriptionHistoryList || []}
                        ...
                        <Box>
                            ...
                            <Typography sx={Typo1}>{`Show ${page}-${size} of ${totalSubscriptionHistoryCount} entries`}</Typography>
                        </Box>
                        <Box sx={{mt: {lg: "0px", md: "0px", sm: "0px", xs: "30px"}}}>
                            <Pagination
                            sx={paginationSx}
                            count={totalSubscriptionHistoryCount>size? totalSubscriptionHistoryCount%size !==0? Math.round(totalSubscriptionHistoryCount/size)+1 : Math.round(totalSubscriptionHistoryCount/size) : 1}
                            page={page}
