...
function SimpleDialog(props) {
  const { onClose, selectedValue, open,  title,isLoading, subTitle = "customers subscription will be canceled, are you sure?", BtnmarginTop,handleOnClick } = props;
      ...
      <Box sx={{ paddingTop: "32px", paddingBottom: "32px" }}>
        ...
        >
          ...
          <Box sx={{ display: "flex", justifyContent: "center", mt: BtnmarginTop ? BtnmarginTop : "16px" }}>
            ...
            <Box ml="24px">
              <Button2 variant="contained" disabled={isLoading} onClick={handleOnClick}>Yes, sure</Button2>
            </Box>
          </Box>
        </Box>
      </Box>
...
export default function CancelSubscriptionComp({ open, setOpen, title, subTitle, BtnmarginTop, handleOnClick, isLoading }) {
  // const [open, setOpen] = React.useState(false);

  const handleClose = (value) => {
    if(!isLoading) setOpen(null);
  };

  return (
    <div>
      <SimpleDialog open={open} onClose={handleClose} isLoading={isLoading} title={title} subTitle={subTitle} BtnmarginTop={BtnmarginTop} handleOnClick={handleOnClick}/>
    </div>
  );
}
