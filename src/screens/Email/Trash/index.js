...
import { useGetAllTrashEmailsMutation, useGetDeleteAllEmailsMutation, useGetUpdateEmailstatusMutation } from '../../../api/email';
...
const Trash = () => {
    ...
    const [deleteAllEmails, responsedata] = useGetDeleteAllEmailsMutation()
    
    const [showTextarea, setShowTextarea] = useState(false);
    const [email, setEmail] = useState("");

    const [allTrashEmailListData, setAllTrashEmailListData] = useState([]);

    const [getAllTrashEmails, resdata] = useGetAllTrashEmailsMutation()
...
const getAllTrashEmailsList = ()=> {
    getAllTrashEmails({ page:1, size:10 }).then((res) => {
      if (res.data){
        console.log("res.data getAllTrashEmailsList ", res?.data);
        setAllTrashEmailListData(res?.data)
      }
    });
  }
  ...
  const updateEmailStatusFunc = (id,status)=> {
    updateEmailStatus({ id:id, status:status }).then((res) => {
      if (res.data){
        ...
        getAllTrashEmailsList()
      }
    });
  }
  
  const deleteAllEmailsFunc = ()=> {
    deleteAllEmails().then((res) => {
      if (res.data){
        console.log("res.data updateEmailStatusFunc ", res?.data);
        getAllTrashEmailsList()
      }
  ...
  useEffect(() => {
    ...
    // console.log("allTrashEmailListData => ",allTrashEmailListData?.data)
    console.log("allTrashEmailListData => ",allTrashEmailListData)
    ...
    <>
     {
        ...
        <Grid container gap={1} sx={{justifyContent:"space-between",minHeight:"85vh"}}>
            <Grid item xs={5} sm={5}>
                <Box>
                    <IconButton onClick={()=> deleteAllEmailsFunc()}>
                        <DeleteForeverIcon sx={{fontSize:"18px"}} />
                    </IconButton>
                </Box>
                {
                    // Array.isArray(allTrashEmailListData?.data) && allTrashEmailListData?.data?.map((item,index)=> (
                    Array.isArray(allTrashEmailListData?.data) && allTrashEmailListData?.data?.length>0? 
                    allTrashEmailListData?.data?.map((item,index)=> (
