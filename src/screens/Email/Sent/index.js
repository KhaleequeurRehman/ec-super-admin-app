...
import { useGetAllSentEmailsMutation, useGetUpdateEmailstatusMutation } from '../../../api/email';
...
const Sent = () => {
    ...
    const [allSentEmailListData, setAllSentEmailListData] = useState([]);

    const [getAllSentEmails, resdata] = useGetAllSentEmailsMutation()
...
const getAllSentEmailsList = ()=> {
    getAllSentEmails({ page:1, size:10 }).then((res) => {
      if (res.data){
        console.log("res.data getCustomersList ", res?.data);
        setAllSentEmailListData(res?.data)
      }
    });
  }
  ...
  useEffect(() => {
    ...
    // console.log("allSentEmailListData => ",allSentEmailListData?.data)
    console.log("allSentEmailListData => ",allSentEmailListData)
    ...
    <>
     {
        ...
        <Grid container gap={1} sx={{justifyContent:"space-between",minHeight:"85vh"}}>
            <Grid item xs={5} sm={5}>
            {
                    Array.isArray(allSentEmailListData?.data) && allSentEmailListData?.data?.length>0  ?allSentEmailListData?.data?.map((item,index)=> (
                ...
                {/* {
                    Array.isArray(allSentEmailListData?.data) && allSentEmailListData?.data?.map((item,index)=> (
                        <Box sx={{mb:"15px"}} key={index}>
                            <MessageUserInfoCard 
                            imageSrc={`${item?.image !== undefined ? item?.image : "assets/images/userWithiMacIconOrange.png"}`} 
                            name={item?.from} 
