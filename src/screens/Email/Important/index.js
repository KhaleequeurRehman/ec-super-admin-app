...
import { useGetAllImportantEmailsMutation, useGetUpdateEmailstatusMutation } from '../../../api/email';
...
const Important = () => {
    ...
    const [allImportantEmailListData, setAllImportantEmailListData] = useState([]);

    const [getAllImportantEmails, resdata] = useGetAllImportantEmailsMutation()
...
const getAllImportantEmailsList = ()=> {
    getAllImportantEmails({ page:1, size:10 }).then((res) => {
      if (res.data){
        console.log("res.data getAllImportantEmailsList ", res?.data);
        setAllImportantEmailListData(res?.data)
      }
    });
  }
  ...
  useEffect(() => {
    ...
    // console.log("allImportantEmailListData => ",allImportantEmailListData?.data)
    console.log("allImportantEmailListData => ",allImportantEmailListData)
    ...
    <>
     {
        ...
        <Grid container gap={1} sx={{justifyContent:"space-between",minHeight:"85vh"}}>
            <Grid item xs={5} sm={5}>
            {
                    // Array.isArray(allImportantEmailListData?.data) && allImportantEmailListData?.data?.map((item,index)=> (
                    Array.isArray(allImportantEmailListData?.data) && allImportantEmailListData?.data?.length>0  ? 
                    allImportantEmailListData?.data?.map((item,index)=> (
