...
import { useAddUserRoleMutation } from "api/roles";
...
export const ModulePermission = ({ hasRoleField, hasTextBtn, hasSubmitBn, onClickFunc, handleCheckCheckbox }) => {
  ...
  const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
    ...
    onSubmit: (values, action) => {
      ...
      addRole(values).then((res)=>{
       if(res.data) swal("Successfully",res.data.message,"success")
      })
      action.resetForm();
    },
  });
      ...
      <MainBox>
        <Grid container spacing={1} sx={{}}>
          {checkboxData.map((item, i) => {
            return (
              //<>
              <Grid xl={2} item lg={12 / 5} md={3} sm={6} xs={12} key={i}>
