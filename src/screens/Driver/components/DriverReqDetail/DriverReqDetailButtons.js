...
export const DriverReqDetailButtons = ({data,refetchDriverDetailData}) => {

  const [updateDriverRequestStatus, resdata] = useUpdateDriverRequestStatusMutation()

  const updateStatus = (newdata)=> {
    updateDriverRequestStatus(newdata).then((res) => {
      if (res.data){
        console.log("res.data updateDriverRequestStatusData ", res?.data);
      }
    })
  }
      ...
      >
        ...
        >
          ...
          onClick={()=> {
            ...
            updateStatus({id:"6393157fb52b11dc22609461",status:"approved"})
            // updateStatus({id:data?._id,status:"approved"})
            setTimeout(()=> refetchDriverDetailData(),1000)
          }}>
            Approve
          </Button1>
        </Box>
        ...
        >
          ...
          onClick={()=> {
            console.log('data?._id ',data?._id); 
            updateStatus({id:"6393157fb52b11dc22609461",status:"revision"})
        ...
        <Box sx={{ mb: { lg: "0px", md: "0px", sm: "10px", xs: "10px" } }}>
          ...
          onClick={()=> {
            console.log('data?._id ',data?._id); 
            updateStatus({id:"6393157fb52b11dc22609461",status:"rejected"})
